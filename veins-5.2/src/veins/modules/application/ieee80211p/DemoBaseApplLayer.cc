...
void DemoBaseApplLayer::handlePositionUpdate(cObject* obj)
{
    ...
    // Try this instead of getCurrentSpeed()
    TraCIMobility* traciMobility = dynamic_cast<TraCIMobility*>(mobility);
    if (traciMobility) {
        // Get the current heading (direction) of the vehicle
        Coord heading = mobility->getCurrentDirection();
        double speed = traciMobility->getSpeed(); // Assuming getSpeed returns a double

        // Create a velocity vector by multiplying speed by heading
        curSpeed = Coord(heading.x * speed, heading.y * speed, heading.z * speed);

        EV_DEBUG << "Speed: " << speed << " m/s, heading: " << heading.x << "," << heading.y
                << " -> curSpeed: " << curSpeed.x << "," << curSpeed.y << std::endl;
    } else {
